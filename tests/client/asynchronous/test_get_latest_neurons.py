...
from pylon._internal.common.requests import GetLatestNeuronsRequest
...
@pytest.mark.asyncio
async def test_async_client_get_latest_neurons_success(
    async_client, service_mock, neurons_json, expected_neurons_response
):
    route = service_mock.get("/api/v1/neurons/latest")
    route.mock(return_value=Response(status_code=codes.OK, json=neurons_json))

    async with async_client:
        response = await async_client.request(GetLatestNeuronsRequest())

    assert response == expected_neurons_response


@pytest.mark.asyncio
async def test_async_client_get_latest_neurons_empty_neurons(async_client, service_mock):
    neurons_json = {
...
@pytest.mark.asyncio
async def test_async_client_get_latest_neurons_retries_success(async_client, service_mock, neurons_json):
    service_mock.get("/api/v1/neurons/latest").mock(
        side_effect=[
...
@pytest.mark.asyncio
async def test_async_client_get_latest_neurons_request_error(async_client, service_mock):
    ...
    async with async_client:
        with pytest.raises(PylonRequestException, match="An error occurred while making a request to Pylon API."):
            await async_client.request(GetLatestNeuronsRequest())


@pytest.mark.asyncio
async def test_async_client_get_latest_neurons_response_error(async_client, service_mock):
    service_mock.get("/api/v1/neurons/latest").mock(return_value=Response(status_code=codes.INTERNAL_SERVER_ERROR))
